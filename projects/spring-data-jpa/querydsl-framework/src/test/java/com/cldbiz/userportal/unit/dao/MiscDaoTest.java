...
public class MiscDaoTest extends BaseRepositoryTest {
	...
	@Test
	public void whenFindMaxPurchaseOrderAccountCommissionedByUser_thenReturnPurchaseOrderAccountDtoDtos() {
		Optional<User> user = userRepository.findById(2L);
		assertThat(user.get()).isNotNull();
		
		UserDto userDto = new UserDto(user.get());
		
		List<PurchaseOrderAccountDto> purchaseOrderAccountDtoDtos = miscDao.findMaxPurchaseOrderAccountCommissionedByUser(userDto);
		productRepository.flush();
		
		assertThat(purchaseOrderAccountDtoDtos).isNotEmpty();
		
		purchaseOrderAccountDtoDtos.forEach(purchaseOrderAccountDto -> {
			assertThat(purchaseOrderAccountDto.getAccountName()).isNotNull();
			assertThat(purchaseOrderAccountDto.getOrderIdentifier()).isNotNull();
			assertThat(purchaseOrderAccountDto.getAmount()).isGreaterThan(BigDecimal.ZERO);
		});
