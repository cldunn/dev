...
public class MiscDaoImpl extends BaseDaoImpl implements MiscDao {
	...
	/*****************************************************************************
	 * Restrict by max uses JPAExpression (formerly subQuery)
	 * Sum uses GroupBy, adhoc uses projections and un-modeled uses from and where
	 *****************************************************************************/

	/********************************************************************************************** 
	 * Collect account name/orderIdentifier/sum(purchaseOrder.lineItems().product.price).as(amount)
	 * for commissioned orders and max(amount) 
	 **********************************************************************************************/
	public List<PurchaseOrderAccountDto> findMaxPurchaseOrderAccountCommissionedByUser(UserDto userDto) {
		QPurchaseOrder purchaseOrder = QPurchaseOrder.purchaseOrder;
		QCommission commission= QCommission.commission;
		QAccount account = QAccount.account;
		QUser user = QUser.user;
		...
		/*
		List<PurchaseOrderAccountDto> results = jpaQueryFactory				
				.from(account, purchaseOrder, commission)
				.select(Projections.bean(PurchaseOrderAccountDto.class,
						purchaseOrder.account.accountName.as("accountName"), 
						purchaseOrder.orderIdentifier.as("orderIdentifier"), 
						purchaseOrder.lineItems.any().product.price.sum().as("amount")))
				.where(commission.userEmail.eq(userDto.getEmail())
						.and(purchaseOrder.orderIdentifier.eq(commission.orderIdentifier))
						.and(account.purchaseOrders.contains(purchaseOrder)))
				.groupBy(account.accountName, purchaseOrder.orderIdentifier, purchaseOrder.lineItems.any().product.price.sum().max())
				.fetch();
		*/
		List<PurchaseOrderAccountDto> results = jpaQueryFactory				
				.from(purchaseOrder, commission)
				.select(Projections.bean(PurchaseOrderAccountDto.class,
						purchaseOrder.account.accountName.as("accountName"), 
						purchaseOrder.orderIdentifier.as("orderIdentifier"), 
						purchaseOrder.lineItems.any().product.price.sum().as("amount")))
				.where(commission.userEmail.eq(userDto.getEmail())
						.and(purchaseOrder.orderIdentifier.eq(commission.orderIdentifier)))
				.groupBy(purchaseOrder.account.accountName, purchaseOrder.orderIdentifier)
				.fetch();

		/* stream building map(accountName, {account, orderIdentifier, maxAmount}) and return values */
		/* faster performance than subquery in where to get orderIdentifier with max amount for each account*/
		
		List<PurchaseOrderAccountDto> results2 = jpaQueryFactory				
				.from(purchaseOrder, commission)
				.select(Projections.bean(PurchaseOrderAccountDto.class,
						purchaseOrder.account.accountName.as("accountName"), 
						purchaseOrder.orderIdentifier.as("orderIdentifier"), 
				...
				.where(commission.userEmail.eq(userDto.getEmail())
						.and(purchaseOrder.orderIdentifier.eq(commission.orderIdentifier))
						.and(purchaseOrder.lineItems.any().product.price.sum().goe(100L)))
				.groupBy(purchaseOrder.account.accountName, purchaseOrder.orderIdentifier)
				.fetch();
